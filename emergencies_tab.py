...
def mark_resolved():
    ...
    try:
        ...
        messagebox.showerror('Please Select a Refugee', 'Please select a refugee.')
    else:
...
def emerg_display(x, username):
    ...
    refugee_viewer = LabelFrame(emerg_ref_tab, width=600, height=500, text='Medical Emergencies', bg='#F2F2F2')
    refugee_viewer.pack()
    ...
    update_treeview_emerg()
    Button(emerg_ref_tab, text = 'Mark Emergency as Resolved' ,command= mark_resolved).pack()
    treeview.pack()
