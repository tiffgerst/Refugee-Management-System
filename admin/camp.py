...
def update_options(*args):
    ...
    for country in countries:
        menu.add_command(
            label=country, command=lambda nation=country: camp_country.set(nation))


def add_camp_window(**kwargs):
    global data
    data = {'Europe':
            ['Albania', 'Andorra', 'Armenia', 'Austria', 'Azerbaijan', 'Belarus', 'Belgium', 'Bosnia and Herzegovina', 'Bulgaria', 'Croatia', 'Cyprus', 'Czechia', 'Denmark', 'Estonia', 'Finland', 'France', 'Georgia', 'Germany', 'Greece', 'Hungary', 'Iceland', 'Ireland', 'Italy', 'Kazakhstan', 'Kosovo', 'Latvia',
                'Liechtenstein', 'Lithuania', 'Luxembourg', 'Malta', 'Moldova', 'Monaco', 'Montenegro', 'Netherlands', 'North Macedonia', 'Norway', 'Poland', 'Portugal', 'Romania', 'Russia', 'San Marino', 'Serbia', 'Slovakia', 'Slovenia', 'Spain', 'Sweden', 'Switzerland', 'Turkey', 'Ukraine', 'United Kingdom', 'Vatican City'],
            'Asia':
            ['Afghanistan', 'Armenia', 'Azerbaijan', 'Bahrain', 'Bangladesh', 'Bhutan', 'Brunei', 'Cambodia', 'China', 'Cyprus', 'East Timor', 'Egypt', 'Georgia', 'India', 'Indonesia', 'Iran', 'Iraq', 'Israel', 'Japan', 'Jordan', 'Kazakhstan', 'Kuwait', 'Kyrgyzstan', 'Laos', 'Lebanon', 'Malaysia', 'Maldives',
             'Mongolia', 'Myanmar', 'Nepal', 'North Korea', 'Oman', 'Pakistan', 'Palestine', 'Philippines', 'Qatar', 'Russia', 'Saudi Arabia', 'Singapore', 'South Korea', 'Sri Lanka', 'Syria', 'Taiwan', 'Tajikistan', 'Thailand', 'Turkey', 'Turkmenistan', 'United Arab Emirates', 'Uzbekistan', 'Vietnam', 'Yemen'],
            'Africa':
                ['Algeria', 'Angola', 'Benin', 'Botswana', 'Burkina Faso', 'Burundi', 'Cabo Verde', 'Cameroon', 'Central African Republic', 'Chad', 'Comoros', 'Congo, Democratic Republic of the', 'Congo, Republic of the', "Cote d'Ivoire", 'Djibouti', 'Egypt', 'Equatorial Guinea', 'Eritrea', 'Eswatini', 'Ethiopia', 'Gabon', 'Gambia', 'Ghana', 'Guinea', 'Guinea-Bissau',
    ...
    for row in rows:
        ...
        if expiration_string == '':
            continue
        expiration_object = datetime.datetime.strptime(
            expiration_string, '%d %b %Y')
    ...
    if len(active_plans) < 1:
        messagebox.showerror('No Plans Available',
                             'Please make an active plan!')
        return
    ...
    Label(add_new_camp_popup, text='Camp Name: *',
          bg='#F2F2F2', font=("Calibri", 15)).pack()
    Entry(add_new_camp_popup, textvariable=camp_name,
          width='30', font=("Calibri", 10)).pack()
    Label(add_new_camp_popup, text='Emergency Plan: *',
          bg='#F2F2F2', font=("Calibri", 15)).pack()
    OptionMenu(add_new_camp_popup, camp_plan, *emergency_plans).pack()
    Label(add_new_camp_popup, text='Country: *',
          bg='#F2F2F2', font=("Calibri", 15)).pack()
    country_options = OptionMenu(add_new_camp_popup, camp_country, '')
    country_options.pack()
    camp_plan.trace('w', update_options)
    Label(add_new_camp_popup, text='City: *',
          bg='#F2F2F2', font=("Calibri", 15)).pack()
    Entry(add_new_camp_popup, textvariable=camp_city,
          width='30', font=("Calibri", 10)).pack()
    Label(add_new_camp_popup, text='Number of Beds: *',
          bg='#F2F2F2', font=("Calibri", 15)).pack()
    Entry(add_new_camp_popup, textvariable=camp_shelter,
          width='30', font=("Calibri", 10)).pack()
    Label(add_new_camp_popup, text="", bg='#F2F2F2').pack()
    Button(add_new_camp_popup, text="Add Camp", height="2",
           width="30", command=add_camp).pack(pady=10)
...
def add_camp():
    ...
    Button(success_popup, text="OK", command=lambda: delete_popups(
        [success_popup, add_new_camp_popup])).pack()


def timetable_summary():

    timetable_labelframe = LabelFrame(
        timetable_pop_up, width=600, height=600, text='Number of volunteers available on: ')
...
def view_timetable():
    ...
    try:
        ...
        messagebox.showerror(
            'Please Select a Camp', 'Please select a camp you wish to view the timetable for!')
        return
    ...
    timetable_viewer = LabelFrame(timetable_pop_up, width=600, height=600,
                                  text='Current Timetable for ' + selected_camp, bg='#F2F2F2')
    timetable_viewer.pack()
    treeview2 = ttk.Treeview(timetable_viewer)
    treescrolly = Scrollbar(
        timetable_viewer, orient='vertical', command=treeview.yview)
    treescrolly.pack(side='right', fill='y')
    treescrollx = Scrollbar(
        timetable_viewer, orient='horizontal', command=treeview.xview)
    treescrollx.pack(side='bottom', fill='x')
    treeview2.configure(xscrollcommand=treescrollx.set,
...
def edit_camp_shelter(sign):
    ...
    try:
        ...
        # No camp selected
        messagebox.showerror('Please Select a Camp',
                             'Please select a camp you wish to edit.')
    ...
    try:
        ...
        messagebox.showerror('Please enter a positive integer',
    ...
    if sign == "+":
        ...
        num_of_refugees = df.loc[df['camp_name'] ==
                                 selected_camp, 'num_relatives'].values
        total_refugees = 0
        for refugee_family in num_of_refugees:
            total_refugees += refugee_family
        df = pd.read_csv('data/camps.csv')
        capacity = df.loc[df['camp_name'] ==
                          selected_camp, 'capacity'].values[0]
        ...
        if shelter_deltas > spare_capacity:
            ...
            if beds_used == '1':
                messagebox.showerror(
                    'Unable to Remove Shelter', 'Unable to remove shelter. The bed you are trying to remove is already in use!')
                return
            messagebox.showerror('Unable to Remove Shelter', 'Unable to remove shelter. ' +
                                 beds_used + " of the beds you are tring to remove are already in use!")
            return
        df.loc[df['camp_name'] == selected_camp, 'capacity'] -= shelter_deltas
...
def delete_camp():
    ...
    try:
        ...
        messagebox.showerror('Please Select a Camp',
                             'Please select a camp you wish to delete.')
    else:
...
def main(x):
    ...
    camp_viewer = LabelFrame(admin_camp_tab, width=600,
                             height=300, text='Current Camps:', bg='#F2F2F2')
    camp_viewer.pack()
    treeview = ttk.Treeview(camp_viewer)

    # displays the scroll bars for horizontal and vertical scrolling
    treescrolly = Scrollbar(
        camp_viewer, orient='vertical', command=treeview.yview)
    treescrolly.pack(side='right', fill='y')
    treescrollx = Scrollbar(
        camp_viewer, orient='horizontal', command=treeview.xview)
    treescrollx.pack(side='bottom', fill='x')
    ...
    Label(camp_viewer, bg='#F2F2F2', text='Search by Camp Name:',
    ...
    Button(admin_camp_tab, text='Add a new camp',
           command=add_camp_window).pack()
    Button(admin_camp_tab, text='Delete camp', command=delete_camp).pack()
    Button(admin_camp_tab, text='View Available Volunteers',
           command=view_timetable).pack()
    ...
    Label(admin_camp_tab, text='Increase or Decrease Camp Capacity:',
          width='50', font=('Calibri', 10)).pack()
    Button(shelter_frame, text='+',
           command=lambda: edit_camp_shelter('+')).pack(side=LEFT)
    Button(shelter_frame, text='-',
           command=lambda: edit_camp_shelter('-')).pack(side=LEFT)
