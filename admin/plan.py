...
def summary_popup():
    ...
    try:
        ...
        # No plan selected
        messagebox.showerror('Please Select a Plan',
                             'Please select a plan you wish to edit.')
    else:
        summary_messagebox = Toplevel(emergencyplan_tab)
        Label(summary_messagebox,
              text=f"Would you like to view or \n download {selected_plan}'s summary").pack()
        Button(summary_messagebox, text='View',
               command=lambda: view(selected_plan)).pack(side=LEFT)
        Button(summary_messagebox, text='Download',
               command=lambda: download(selected_plan)).pack(side=LEFT)
...
def delete_or_close_plan(operation):
    ...
    try:
        ...
        messagebox.showerror('Please Select a Plan',
                             'Please select a plan you wish to '+operation+'.')
        return
    ...
    if confirmation == 'yes':
        ...
        camps = df2.loc[df2['emergency_plan_name']
                        == selected_plan, 'camp_name'].values
...
def modify_plan_window(add):
    ...
    names = ['Name: *', 'Type: *', 'Description: *', 'Continent: *',
             'Start Date: *\n(format: 1 Jul 2019)', 'End Date\n(format: 1 Jul 2019)']
    textvariables = [plan_name, plan_type, plan_description,
                     plan_location, plan_start_date, plan_end_date]
    ...
    if add == True:
        ...
        if plan_name in expired_plans:
            messagebox.showerror('Unable to edit Plan',
                                 'Cannot Edit Expired Plans')
            return
        ...
        # If not change it into our desired format
            ...
            defaults[-1] = datetime.strftime(
                pd.to_datetime(defaults[-1]), "%d %b %Y")

        # Change start date to the desired format
        defaults[-2] = datetime.strftime(
            pd.to_datetime(defaults[-2]), "%d %b %Y")
    ...
    subregions = ['Europe', 'Oceania', 'North America',
    ...
    for i, (name, textvariable) in enumerate(zip(names, textvariables)):
        ...
        if textvariable == plan_location:
            ...
            label = Entry(modify_popup, textvariable=textvariable,
                          width='30', font=("Calibri", 10))
    ...
    Button(modify_popup, text="Confirm", height="2", width="30",
           command=lambda: modify_table(add=add)).pack(pady=10)
...
def modify_table(add):
    ...
    res = is_valid_plan(modify_popup, plan_na, plan_ty,
                        plan_loc, plan_desc, plan_start, plan_end)
    if not res:
        messagebox.showerror(
            'Invalid Entries', "End date must not be before start date or today's date")
        return
    else:
        plan_start, plan_end = res
    ...
    if add == True:
        ...
        if len(df.loc[df['name'] == plan_na]) != 0:
            messagebox.showerror(
                'Invalid Plan Name', 'This plan name has already been taken', parent=modify_popup)
            return
        ...
        updated_row = [plan_na, plan_ty, plan_desc,
                       plan_loc, plan_start, plan_end]
    ...
    if add == True:
        admin.camp.add_camp_window(default=plan_na)
    ...
    if add == True:
        ...
        text = "edit"
    Label(success_popup, text="Plan "+text +
          " was successful. Please add a camp for this plan!", fg='green').pack()
...
def main(x):
    ...
    emergencyplan_viewer = LabelFrame(
        emergencyplan_tab, width=600, height=300, text='Emergency Plans:', bg='#F2F2F2')
    ...
    treescrolly = Scrollbar(emergencyplan_viewer,
                            orient='vertical', command=treeview.yview)
    treescrolly.pack(side='right', fill='y')
    treescrollx = Scrollbar(emergencyplan_viewer,
                            orient='horizontal', command=treeview.xview)
    ...
    Label(emergencyplan_viewer, bg='#F2F2F2',
          text='Search by Plan Name:', font=('Calibri', 14)).pack()
    search_bar.pack()
    ...
    Button(emergencyplan_tab, text='Add a new plan',
           command=lambda: modify_plan_window(add=True)).pack()

    Button(emergencyplan_tab, text='Edit Plan',
           command=edit_plan_confirm).pack()
