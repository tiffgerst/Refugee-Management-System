...
def volunteer_activation():
    ...
    try:
        ...
        messagebox.showerror('Please Select a Volunteer',
                             'Please select a Volunteer you wish to activate/deactivate.')
    else:
        df = pd.read_csv('data/volunteers.csv')
        username_index = df.index[df['username']
                                  == selected_volunteer].tolist()
...
def delete_volunteer_confirm():
    ...
    try:
        ...
        messagebox.showerror('Please Select a Volunteer',
                             'Please select a Volunteer you wish to delete.')
    else:
...
def main(x):
    ...
    volunteer_viewer = LabelFrame(
        volunteer_tab, width=600, height=300, text='All Volunteers:', bg='#F2F2F2')
    ...
    treescrolly = Scrollbar(
        volunteer_viewer, orient='vertical', command=treeview.yview)
    treescrolly.pack(side='right', fill='y')
    treescrollx = Scrollbar(
        volunteer_viewer, orient='horizontal', command=treeview.xview)
    ...
    Label(volunteer_viewer, bg='#F2F2F2',
          text='Search by Username:', font=('Calibri', 14)).pack()
    search_bar.pack()
    ...
    Button(volunteer_tab, text='Activate/Deactive Volunteer',
